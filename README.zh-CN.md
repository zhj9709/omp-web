...
- **网页配置**：无需离开 OMP Web，即可管理 Provider 登录和 API Key、模型、模型测试、插件包及技能。
- **英文和简体中文界面**：OMP Web 首次打开时跟随浏览器语言，也可从顶部栏切换语言。

## 快速开始

OMP Web 要求 Node.js 22.19.0 或更高版本。先用 `node --version` 检查版本，然后运行：

```bash
npx @agegr/omp-web@latest
```

服务就绪后，命令行会尝试自动打开浏览器。如果没有打开，请访问 [http://127.0.0.1:30142](http://127.0.0.1:30142)。OMP Web 默认仅监听 `127.0.0.1`。
...
更新前先用 `Ctrl+C` 停止正在运行的进程，再次执行同一条安装命令。卸载时运行 `npm uninstall -g @agegr/omp-web`。

## 配置

端口和主机名以命令行参数为准，优先于对应的环境变量。`--no-open` 与 `OMP_WEB_NO_OPEN=1` 中任意一个都会关闭自动打开浏览器。
...
```bash
OMP_WEB_PASSWORD='足够长的随机密码' omp-web --hostname 0.0.0.0
```

Basic Auth 不会加密传输中的密码。不要通过明文 HTTP 将 OMP Web 暴露到互联网；远程访问应使用可信反向代理提供 HTTPS，或通过可信 VPN。如果反向代理传递外部主机名，请把该名称精确加入 `OMP_WEB_ALLOWED_HOSTS`。这个白名单不会改变 OMP Web 的监听地址。
