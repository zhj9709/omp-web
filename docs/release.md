...
- npm package: `@agegr/omp-web`
- GitHub Release: `agegr/omp-web`
...
- npm is authenticated as an account that can publish `@agegr/omp-web`.
...
- If `npm view @agegr/omp-web version` briefly shows the previous version, check the exact version instead:

```bash
npm view @agegr/omp-web@<version> version --registry https://registry.npmjs.org/
npm view @agegr/omp-web versions --json --registry https://registry.npmjs.org/
```
...
gh release view v<version> --repo agegr/omp-web
```
...
- 发布 npm 包 `@agegr/omp-web@<version>`。
...
- Published npm package `@agegr/omp-web@<version>`.
```
...
```bash
gh release edit v<version> --repo agegr/omp-web --notes-file - <<'EOF'
## 中文
