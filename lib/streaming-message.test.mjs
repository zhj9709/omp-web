...
test("merges a deltaBatch into one accumulated text block", () => {
  let state = streamReducer(INITIAL_STREAMING_STATE, { type: "start" });
  state = snapshot(state, assistant());
...
test("applies a deltaBatch with mixed thinking and text deltas in order", () => {
  let state = streamReducer(INITIAL_STREAMING_STATE, { type: "start" });
  state = snapshot(state, assistant());
  state = streamReducer(state, {
    type: "deltaBatch",
    events: [
      ...
      { type: "thinking_end", contentIndex: 0, content: "Plan." },
