...
test("opens the transport before a slow session is ready and snapshots after subscribing", async (t) => {
  ...
  // Teardown guarantee: if an assertion below throws, the stream is never
  // aborted by the test body and the 30s heartbeat interval keeps the test
  // process alive forever. Abort unconditionally after the test.
  t.after(async () => {
  ...
  assert.deepEqual(decodeData(await readWithin(reader)), {
    ...
    assistantMessageEvent: {
      type: "text_delta",
      delta: "!",
      // toClientAgentEvent normalizes contentIndex (OMP sends it as a
      // string); a missing index becomes 0.
      contentIndex: 0,
    },
  });
