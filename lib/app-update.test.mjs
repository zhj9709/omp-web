...
const { getOmpWebReleaseUrl, isNewerStableVersion } = await jiti.import("./app-update.ts");

test("detects newer stable OMP Web versions", () => {
...
test("builds a release-notes URL only for stable versions", () => {
  assert.equal(
    getOmpWebReleaseUrl("0.8.8"),
    "https://github.com/agegr/omp-web/releases/tag/v0.8.8",
  );
  assert.equal(getOmpWebReleaseUrl("0.8.8-beta.1"), null);
});
