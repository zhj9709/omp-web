...
test("projects ttsr_triggered rules down to names for the wire", () => {
  assert.deepEqual(toClientAgentEvent({
    type: "ttsr_triggered",
    rules: [
      { name: "no-secrets", condition: "sk-[A-Za-z0-9]+" },
      { name: "no-hallucinated-api", scope: "text" },
    ],
  }), {
    type: "ttsr_triggered",
    rules: ["no-secrets", "no-hallucinated-api"],
  });

  assert.deepEqual(toClientAgentEvent({ type: "ttsr_triggered", rules: [] }), {
    type: "ttsr_triggered",
    rules: [],
  });
});
...
test("normalizes string contentIndex to numbers on message_update", () => {
  const partial = assistantMessage("Hello");

  function project(contentIndex) {
    return toClientAgentEvent({
      type: "message_update",
      message: partial,
      assistantMessageEvent: {
        type: "text_delta",
  ...
  assert.equal(project("0").assistantMessageEvent.contentIndex, 0);
  assert.equal(project("1").assistantMessageEvent.contentIndex, 1);
  assert.equal(project(2).assistantMessageEvent.contentIndex, 2);
});
