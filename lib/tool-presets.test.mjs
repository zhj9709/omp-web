...
test("classifies presets that remove write/bash tools as restrictive", () => {
  assert.equal(isRestrictiveToolRequest(PRESET_NONE), true);
  assert.equal(isRestrictiveToolRequest(PRESET_READ_ONLY), true);
  assert.equal(isRestrictiveToolRequest(["read", "grep", "find", "ls"]), true);
  assert.equal(isRestrictiveToolRequest(PRESET_DEFAULT), false);
  assert.equal(isRestrictiveToolRequest(PRESET_FULL), false);
  assert.equal(
    isRestrictiveToolRequest(["read", "bash", "edit", "write", "glob", "grep"]),
    false,
  );
});
